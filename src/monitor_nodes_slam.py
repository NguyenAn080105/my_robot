...
def cmd_vel_callback(msg):
    ...
    # Chỉ phát hiện khi robot thực sự di chuyển
...
def main():
    ...
    # Init node
    rospy.init_node('slam_performance_monitor', anonymous=True)
    ...
    print(f"--- SLAM Performance Monitor (RAW DATA) ---")
    print(f"Monitoring: {args.nodes}")
    print("Status: Waiting... (Press ENTER to mark Loop Closure/Finish)")
    ...
    df.to_csv(f"ros_benchmark_slam_{timestamp}.csv", index=False)
    
    # [QUAN TRỌNG] Đã xóa đoạn code Smoothing ở đây. Sử dụng trực tiếp dữ liệu thô.
    ...
    # --- Plot 1: CPU (RAW) ---
    for node in args.nodes:
        # Vẽ dữ liệu thô (bỏ _Smooth)
        ax1.plot(df['Time'], df[f'{node}_CPU'], label=node, alpha=0.7, linewidth=1.5, linestyle='-')
    
    # Vẽ Total đậm
    #ax1.plot(df['Time'], df['Total_CPU'], label='TOTAL SYSTEM', color='black', linewidth=2.5)
    
    draw_markers(ax1)
    
    max_cpu_val = df['Total_CPU'].max()
    ...
    # [ĐÃ BỔ SUNG] Vẽ Total RAM màu đen
    ...
    plt.savefig(f"benchmark_slam_{timestamp}.png")
    print(f"\nDone! Chart saved at: benchmark_slam_{timestamp}.png")
    plt.show()
